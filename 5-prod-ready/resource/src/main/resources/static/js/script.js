...
function retieveCurrentWeather(node) {
    $.ajax({
        ...
        success: function(data) {
            ...
            retrieveSuggestion(node, data.temperature.toFixed(1), data.weatherIcon);
        }
    });
}

function retrieveSuggestion(node, temperature, weather) {
    $.ajax({
        url: '/suggestion/'+temperature+'/'+weather,

        success: function(data) {
            node.after('<div>Wear this: <a href="'+ data.link +'"> <img border="0" alt="adidas" src="'+ data.image +'" width="64" height="64"></div>');
       }
    });
}
...
function loadMembers() {
    $.ajax({
        ...
        dataType: 'json',
        success: function(data) {
            renderMembers(data._embedded.members);
        }
    });
}
